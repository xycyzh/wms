...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".btn_audit").click(function () {
                ...
                createDialog("温馨提示", "确定要审核这条入库订单吗?", "warning", function () {
                    //发起ajax请求
                    $.ajax({
                        url: url,
                        type: "get",
    ...
    <title>叩丁狼教育PSS（演示版）-入库订单管理</title>
    <style>
        .alt td {
                            ...
                            <td>
                                <c:if test="${list.status==0}">
                                    <a href="javascript:;" class="btn_audit"
                                       data-url="/stockIncomeBill/audit.do?id=${list.id}">审核</a>
                                    <a href="javascript:;" class="btn_redirect"
                                       data-url="/stockIncomeBill/input.do?id=${list.id}">编辑</a>
                                    <a href="javascript:;" class="btn_delete"
                                       data-url="/stockIncomeBill/delete.do?id=${list.id}"
                                       data-redirect="/stockIncomeBill/list.do">删除</a>
                                </c:if>
                                <c:if test="${list.status==1}">
                                    <a href="javascript:;" class="btn_redirect"
                                       data-url="/stockIncomeBill/view.do?id=${list.id}">查看</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:forEach>
